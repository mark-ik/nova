...
#[cfg(not(all(target_family = "wasm", feature = "single-threaded")))]
mod generated;
#[cfg(all(target_family = "wasm", feature = "single-threaded"))]
mod wasm_single_thread;
#[cfg(all(target_family = "wasm", feature = "single-threaded"))]
use wasm_single_thread as generated;

#[cfg(not(all(target_family = "wasm", feature = "single-threaded")))]
mod unordered_copy;
#[cfg(all(target_family = "wasm", feature = "single-threaded"))]
mod wasm_unordered_copy;
#[cfg(all(target_family = "wasm", feature = "single-threaded"))]
use wasm_unordered_copy as unordered_copy;
...
unsafe fn enter_racy_memory<T: RacyStorage>(ptr: NonNull<T>) -> RacyPtr<T> {
    #[cfg(all(target_family = "wasm", feature = "single-threaded"))]
    {
        return RacyPtr::from_ptr(ptr.cast());
    }
...
impl<T: RacyStorage> Racy<'_, T> {
    ...
    #[cfg(any(
        ...
        target_arch = "aarch64",
        target_arch = "arm",
        all(target_family = "wasm", feature = "single-threaded")
    ))]
